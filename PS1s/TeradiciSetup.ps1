...
Install-ChocolatyAndPackages {
    ...
    Write-Log "choco Install Microsoft C++ Redistribution"
    choco install -y vcredist-all

    #Write-Log "choco Install Microsoft .Net 3.5"
...
Mount-DataDisk {
    ...
    $count = 0
    $labels = "AvidMedia", "data1", "data2", "data3", "data4", "data5", "data6", "data7", "data8", "data9", "data10"
...
function Mount-AvidInstallDrive 
{
     
    $acctKey = ConvertTo-SecureString -String "re7hgT7kaPTklrBsYHXTLjQ6dVjRKjbXMvTThTXYevYHmJVxmLNrktJ8OHnMrzPbKQPjg9GemjAl6ikACBpXGg==" -AsPlainText -Force

    $credential = New-Object System.Management.Automation.PSCredential -ArgumentList "Azure\avidmarketplacesa", $acctKey

    New-PSDrive -Name Z -PSProvider FileSystem -Root "\\avidmarketplacesa.file.core.windows.net\install" -Credential $credential -Persist 

    $sourceMediaRoot = "Z:\Avid Sample Content\Avid MediaFiles"
    $sourceProjectRoot = "Z:\Avid Sample Content\Avid Project"
    $destinationRoot = "F:\"

    Copy-Item -Path $sourceMediaRoot -Destination $destinationRoot -recurse -Force -Verbose
    Copy-Item -Path $sourceProjectRoot -Destination $destinationRoot -recurse -Force -Verbose
...
try {
    ...
    if ($true) 
    {
        ...
        Write-Log ".\CustomDataSetupScript.ps1 -MediaComposerURL $MediaComposerURL -TeradiciURL $TeradiciURL -NvidiaURL $NvidiaURL -TeradiciKey $TeradiciKey"
        ...
        Write-Log "Mount Avid Install Drive"
        Mount-AvidInstallDrive 
